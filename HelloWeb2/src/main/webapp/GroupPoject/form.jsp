...
<script>
  ...
  $(document).ready(function (){ //페이지가 완전히 로딩되면 실행
    const form = $('#orderform'); // <form id="orderform">을 jQuery 객체로 가져오고, form이라는 변수로 사용
    
    //form.on('submit', function(e) { // 이벤트 핸들러에서 function 키워드 사용
    //console.log('submit 이벤트 잡음...');
      
    //orderform이 <form>이니까 form안에 있는 name="user"
    //(input 태그의 name 속성이 name, userId, email ... 등 에서 가져오는 거)
...
<div class="container">
  <div class="child">
    <form id="orderform" method="post" action="insert.jsp">
      ...
      <fieldset>
        ...
        <div class="field">
           ...
           <input type="radio" name="type" value="Starter">&nbsp;&nbsp;
    ...
    <br>
      <fieldset>
        <legend>주문 수량</legend>
        <div class="field">
           <select id="orderSelectAmount" name="orderQuantity">
           ...
           <input id="orderInputQuantity" type="text" size="10" placeholder="직접 작성">
        </div>
